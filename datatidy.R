...
df1 <- df1 %>% 
  select(., c("country", "year", "v2x_polyarchy", # Electoral democracy index
              ...
              "group", # Is the state within the 80th percentile of political violence?
...
lm3 <- lm_robust(v2excrptps_rev ~ v2x_clphy + log(gdppc) + v2x_clphy:log(gdppc),
                 subset = (df1$group == 1 &
                             df1$isdemocracy == 0),
                 clusters = country,
                 data = df1)

lm4 <- lm_robust(v2excrptps_rev ~ v2x_clphy + log(gdppc) + v2x_clphy:log(gdppc) + v2exl_legitperf,
                 subset = (df1$group == 1 &
                             df1$isdemocracy == 0),
                 clusters = country,
                 data = df1)
...
modelsummary(list(lm1, lm2, lm3, lm4),
             ...
             coef_map = c('v2x_clphy' = 'Physical violence index',
                          'log(gdppc)' = 'GDP per capita (logged)',
                          'v2x_clphy:log(gdppc)' = 'Physical violence X GDP per capita (logged)',
                          'v2exl_legitperf' = 'Performance legitimisation',
                          '(Intercept)' = 'Intercept'),
             stars = c('*' = .1, '**' = .05, '***' = .01),
             gof_omit = 'BIC|Log.Lik.|RMSE|Std.Errors',
             notes = list('Standard errors clustered by country.',
                          'Sample: 80th percentile of the physical violence index among non-democracies')) %>% 
...
modelsummary(list(lm5, lm6, lm7, lm8),
             output = 'kableExtra',
             coef_map = c('v2x_clphy' = 'Physical violence index',
                          'log(gdppc)' = 'GDP per capita (logged)',
                          'v2x_clphy:log(gdppc)' = 'Physical violence X GDP per capita (logged)',
